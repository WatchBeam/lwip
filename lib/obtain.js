...
const decoder = require('bindings')('lwip_decoder');
...
exports.open = function open() {
  getJudge(defs.args.open)(arguments, (source, type, callback) => {
    let opener;
    if (typeof source === 'string') {
      type = type || extname(source).slice(1);
      if (typeof type !== 'string') throw Error('Invalid type');
      opener = getOpener(type);
      readFile(source, (err, imbuff) => {
...
exports.create = function create() {
  getJudge(defs.args.create)(arguments, (width, height, color, callback) => {
