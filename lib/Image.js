...
const { getJudge } = require('./decree');
const defs = require('./defs');
const util = require('./util');
const encoder = require('bindings')('lwip_encoder');
const lwip_image = require('bindings')('lwip_image');
const normalizeColor = util.normalizeColor;

const judges = {
  scale: getJudge(defs.args.scale),
  resize: getJudge(defs.args.resize),
  contain: getJudge(defs.args.contain),
  cover: getJudge(defs.args.cover),
  rotate: getJudge(defs.args.rotate),
  blur: getJudge(defs.args.blur),
  hslaAdjust: getJudge(defs.args.hslaAdjust),
  saturate: getJudge(defs.args.saturate),
  lighten: getJudge(defs.args.lighten),
  darken: getJudge(defs.args.darken),
  fade: getJudge(defs.args.fade),
  opacify: getJudge(defs.args.opacify),
  hue: getJudge(defs.args.hue),
  crop: getJudge(defs.args.crop),
  mirror: getJudge(defs.args.mirror),
  pad: getJudge(defs.args.pad),
  border: getJudge(defs.args.border),
  sharpen: getJudge(defs.args.sharpen),
  paste: getJudge(defs.args.paste),
  clone: getJudge(defs.args.clone),
  extract: getJudge(defs.args.extract),
  toBuffer: getJudge(defs.args.toBuffer),
  writeFile: getJudge(defs.args.writeFile),
  setPixel: getJudge(defs.args.setPixel),
  getPixel: getJudge(defs.args.getPixel),
};
...
exports.Image = class Image {
  ...
  contain() {
    judges.contain(arguments, (width, height, color, inter, callback) => {
  ...
  cover() {
    judges.cover(arguments, (width, height, inter, callback) => {
  ...
  paste() {
    this.__lock();
    try {
      judges.paste(arguments, (left, top, img, callback) => {
  ...
  setPixel() {
    this.__lock();
    try {
      judges.setPixel(arguments, (left, top, color, callback) => {
        if (left >= this.width() || top >= this.height())
          throw Error('Coordinates exceed dimensions of image');
        color = normalizeColor(color);
        if (color.a < 100) this.__trans = true;
  ...
  clone() {
    ...
    // just copy it.
    judges.clone(arguments, callback => {
      // first we retrieve what we need (buffer, dimensions, ...)
  ...
  extract() {
    ...
    judges.extract(arguments, (left, top, right, bottom, callback) => {
      // first we retrieve what we need (buffer, dimensions, ...)
      // synchronously so that the original image doesn't have a chance
